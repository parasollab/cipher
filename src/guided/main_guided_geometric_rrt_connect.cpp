...
#include "guided_geometric_rrt_connect.h"
...
// GoalSampleableRegion is required by RRTConnect to seed the goal tree.
class PositionGoalCondition : public ob::GoalSampleableRegion
{
public:
    PositionGoalCondition(
        ...
        double threshold)
        : ob::GoalSampleableRegion(si), goal_state_(goal_state)
    {
    ...
    {
        si_->getStateSpace()->copyState(st, goal_state_);
    }
...
int main(int argc, char** argv)
{
    ...
    if (do_viz) {
        ...
        std::cout << "[viz] Header written (" << decomp->getNumRegions()
                  << " cells) to " << vizFile << std::endl;
    }
    ...
    for (int r = 0; r < num_robots; ++r) {
        ...
        auto planner = std::make_shared<GuidedGeometricRRTConnect>(robot_si);
