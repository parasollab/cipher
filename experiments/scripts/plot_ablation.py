"""
Plot ablation results for geometric_cipher on boxes_70x70.

Reads experiments/results/ablation.csv and produces an 8-page PDF plus
individual PNGs in experiments/plots/ablation/.
...
def main():
    ...
    default_csv = str(project_root / 'experiments' / 'results' / 'ablation.csv')
    default_out = str(project_root / 'experiments' / 'plots' / 'ablation')

    parser = argparse.ArgumentParser(
        description="Plot ablation results for geometric_cipher on boxes_70x70.",
    ...
    parser.add_argument('--default-region-size', type=float, default=5.0,
                        help="Fixed region_size for ratio-sweep and CBS-vs-A* plots")
    parser.add_argument('--default-ratio', type=float, default=1.3,
    ...
    pdf_path = os.path.join(args.output_dir, 'ablation.pdf')
